...
window.onload = function () {
    ...
    (function () {
                                ...
                                for (j = 0; j < ENEMY_SHOT_MAX_COUNT; j++) {
                                    if (!enemyShot[j].alive) {
                                        if (enemy[i].type == 0) {
                                            ...
                                            enemyShot[j].set(enemy[i].position, { x: 0, y: 1.5 }, 5, 3);
                                        }
        ...
        if(BOSS_MAX_COUNT <= 0){
        if( BOSS_MAX_COUNT <= 0){ BOSS_MAX_COUNT = 0, BOSS_HP = 0 }
        }
        else if(BOSS_MAX_COUNT >=1){
        if (BOSS_HP <= 0){BOSS_HP +=100,BOSS_MAX_COUNT-=1,point +=5000}
        }
...
function ChangeColor() {
    ...
    if ( BOSS_MAX_COUNT == 1){ BOSS_COLOR = 'rgba(255,255,0,1)' , BOSS_SHOT_COLOR = 'rgba(35, 71, 148,0.8)'}
}
